...
def find_candidate_pairs(hashes: np.ndarray, gate: int) -> np.ndarray:
    ...
    for start in range(0, n, chunk):
        ...
        distances = np.bitwise_count(queries ^ index)  # (c, n, 4)
        best = distances.min(axis=2)
        best_rotation = distances.argmin(axis=2)

        query_rows = np.arange(start, stop)[:, np.newaxis]
        within_gate = best <= gate
        upper_triangle = columns > query_rows  # keeps only a < b
        rows, cols = np.nonzero(within_gate & upper_triangle)
        if rows.size == 0:
            continue

        image_a = (start + rows).astype(np.int64)
        image_b = cols.astype(np.int64)
        hamming = best[rows, cols].astype(np.int64)
        rotation_a = np.zeros(rows.size, dtype=np.int64)  # always 0
        rotation_b = _ROTATION_DEGREES[best_rotation[rows, cols]]
        found.append(
            np.stack([image_a, image_b, hamming, rotation_a, rotation_b], axis=1)
        )
...
def embedding_work_set(pairs) -> list:
...
def cosine_for_pairs(pairs, vectors: dict) -> list:
...
def group_pairs(node_count: int, edges) -> list:
...
def assemble_groups(pairs, images: dict) -> list:
    ...
    for component in group_pairs(len(ids), edges):
        ...
        for image_id in members:
            ...
            if image_id == keeper:
                ...
                keeper_key = (min(image_id, keeper), max(image_id, keeper))
                if keeper_key in scored:
                    similarity = scored[keeper_key]
                else:
                    # Joined to the group transitively, with no direct pair
                    # against the keeper. Fall back to its strongest link to
                    # any other member of the group.
                    similarity = 0.0
                    for other in members:
                        other_key = (min(image_id, other), max(image_id, other))
                        if other_key in scored and scored[other_key] > similarity:
                            similarity = scored[other_key]
            files.append(
                {
                    "id": image_id,
                    "path": item["path"],
...
def pick_keeper(items: list) -> int:
    ...
    for i in range(1, len(items)):
        ...
        winner_pixels = winner["width"] * winner["height"]

        if candidate_pixels != winner_pixels:
            if candidate_pixels > winner_pixels:
                best = i
        elif candidate["size"] != winner["size"]:
            if candidate["size"] > winner["size"]:
                best = i
        elif candidate["mtime"] < winner["mtime"]:
            best = i
